...
class firedb_semester(val context: Context, val view: View): local_db(){
    override val TAG = "firedb_semester"
...
open class firedb_setup(): local_db(){
    override val TAG = "firedb_setup"
...
class firedb_task(val context: Context){
            ...
            .addOnSuccessListener {
                ...
                for( week in week_to_day_symbol_list) {
                    for (period in period_list) {
                        ...
                        val class_data = user_data?.get(week_period) as? Map<*, *>
                        if (class_data != null){
                            val class_id = class_data.get("course_id") as String
                            Log.d("hogee", "class_id: $class_id")
                            //class_week_to_day_array += week_period
    ...
    fun get_not_comp_task_list(view: View){
        ...
        for (week in week_to_day_symbol_list) {
            for (period in period_list) {
                val user_class_data = test_course_data_map[week + period] as? MutableMap<String, Any>
